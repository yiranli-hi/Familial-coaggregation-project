---
title: "Recurrence risk ratio to control group"
format: html
...
1.  prepare the data for linear model: age\**Exposure + age\^2 \* Exposure + sex\**Exposure (options: + kinshipSize) kinship size is a categorical variable

2.  linear model function, calculate the RRR and SE, 95% CI, RRR is relative risk ratio to control group.

3.  prepare data for h2 calculation

4.  present results in tables and figures
...
```{r}
# 1. se of x/y
getRRR_SE <- function(x, x_SE, y, y_SE){
  varRR <- (1/y^2) * (x_SE^2) + (x^2/y^4) * (y_SE^2)
  seRR <- sqrt(varRR)
  return(seRR)
}
...
getRRR <- function( Outcome, Exposure, Relative, dt)
  ...
  # NA is only for sex-pecific kinship (brother), not for kinship (sibling) in our analysis data
  dt <- dt[!is.na(get(RRR_var)), ]
  # calcualte individual age
  ...
  if(Relative == "Parent"){
    ...
    dt[NumberAffected>2, NumberAffected := 2]
  ...
  # avoid extreme number in kinship size
  dt[get(count_var)>2, (count_var) := 3]
  if_n_kinship <- dt[,.N, by = count_var][get(count_var) == max(get(count_var)), N]
  if(if_n_kinship <100){
    dt[get(count_var) == max(get(count_var)), (count_var):= get(count_var) -1]
  }
  ...
  denominator <- avg_predictions(model, type = "response")
  ...
  numerator <- avg_predictions(model, variables = "NumberAffected", type = "response") %>% 
