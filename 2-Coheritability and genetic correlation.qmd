...
## Weigthed mean of heritability and genetic correlation

only for first degree relative

```{r}
##############weighted h2################
dt_h2_weighted <- dt_h2 %>% 
  filter(Relation %in% c( "BothParent", "Parent",  "Father", "Mother",
                          "Offspring","Son","Daughter", "Sibling", 
                          "Sister", "Brother")) %>% 
  dplyr::select(-Relation) %>% 
  nest_by(Outcome) %>% 
  mutate(h2_v_weighted = list(
    get_wtmean(V = data)
  )) %>% 
  dplyr::select(Outcome, h2_v_weighted) 

setDT(dt_h2_weighted)
dt_h2_weighted[, c("h2", "h2_L", "h2_H", "h2_SE") := transpose(h2_v_weighted), by =.I]
dt_h2_weighted[,h2_v_weighted:=NULL]
dt_h2_weighted <- dt_h2_weighted %>% as_tibble() 
writexl::write_xlsx(dt_h2_weighted, 
                    "results/result_h2_weighted_allFDR.xlsx")


##############weighted rg################
dt_rg_weighted <- dt_rg %>% 
  dplyr::select(Relation, Outcome, Exposure, rg, rg_L, rg_H, rg_SE) %>% 
  filter(Relation %in% c( "BothParent", "Parent",  "Father", "Mother",
                          "Offspring","Son","Daughter","Sibling", 
                          "Sister", "Brother")) %>% 
  dplyr::select(-Relation)

setDT(dt_rg_weighted)
dt_rg_weighted[, PhenotypePair:= paste(pmin(Outcome, Exposure), pmax(Outcome, Exposure), sep = "-")]

dt_rg_weighted <- dt_rg_weighted %>% as_tibble() %>% 
  dplyr::select(-Outcome, -Exposure)
  
dt_rg_weighted <- dt_rg_weighted %>% 
  nest_by(PhenotypePair) %>% 
  mutate(rg_v_weighted = list(
    get_wtmean(V = data)
  )) %>% 
  dplyr::select(PhenotypePair, rg_v_weighted) 

setDT(dt_rg_weighted)
dt_rg_weighted[, c("rg", "rg_L", "rg_H", "rg_SE") := transpose(rg_v_weighted), by =.I]
dt_rg_weighted[,rg_v_weighted:=NULL]
dt_rg_weighted <- dt_rg_weighted %>% as_tibble() 
writexl::write_xlsx(dt_rg_weighted, 
                    "results/result_rg_weighted_allFDR.xlsx")
```

only for second degree relative

```{r}
##############weighted h2################
dt_h2_weighted <- dt_h2 %>% 
  filter(Relation %in% c("AuntUncle", "Uncle","Aunt",
                          "NieceNephew","Nephew","Niece",
                          "Grandparent", "Grandfather","Grandmother",
                          "Grandchild",  "Grandson", "Granddaughter",
                          "Halfsibling",  "Halfbrother", "Halfsister")) %>% 
  dplyr::select(-Relation) %>% 
  nest_by(Outcome) %>% 
  mutate(h2_v_weighted = list(
    get_wtmean(V = data)
...
dt_h2_weighted <- dt_h2_weighted %>% as_tibble() 
writexl::write_xlsx(dt_h2_weighted, 
                    "results/result_h2_weighted_allSDR.xlsx")
...
dt_rg_weighted <- dt_rg_weighted %>% as_tibble() 
writexl::write_xlsx(dt_rg_weighted, 
                    "results/result_rg_weighted_allSDR.xlsx")

```

for both first degree relative and second degree relative
...
dt_h2_weighted <- dt_h2_weighted %>% as_tibble() 
writexl::write_xlsx(dt_h2_weighted, 
                    "results/result_h2_weighted_all.xlsx")
...
dt_rg_weighted <- dt_rg_weighted %>% as_tibble() 
writexl::write_xlsx(dt_rg_weighted, 
                    "results/result_rg_weighted_all.xlsx")
